...
character class is empty. Since character classes are sets of characters, and
empty class corresponds to the empty set. Since nothing is in the empty set,
`[a&&b]` matches nothing, not even the empty string.
...
* The pattern string is parsed into a structured representation called an
`Ast`. Counted repetitions are not expanded and Unicode character classes are
not looked up in this stage. That is, the size of the `Ast` is proportional
to the size of the pattern with "reasonable" constant factors. In other words,
one can reasonably limit the memory used by an `Ast` by limiting the length of
the pattern string.
...
Unicode character classes---can be quite high. Still though, the memory used
by an `Hir` can be reasonably limited by limiting the length of the pattern
string.
...
[`Regex::captures_iter`] actually have worst case `O(m^2 * n)` search time.
This is because `find_iter` runs many searches, and each search takes worst
case `O(m * n)` time. What's the worst case number of searches that can be
run? That's equal to `m`. Thus, iteration of all matches in a haystack has
worst case `O(m^2 * n)`. A good example of a pattern that exhibits this is
`(?:A+){1000}|`.
...
This module provides a nearly identical API via [`Regex`](bytes::Regex) to the
one found in the top-level of this crate. There are two important differences:
