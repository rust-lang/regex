...
//!
//! TODO: Adapt documentation from `src/bytes.rs`?
...
///     fields.push((f1, std::str::from_utf8(f2)?.parse()?, std::str::from_utf8(f3)?.parse()?));
/// }
...
impl Regex {
    ...
    pub fn find_iter<'r, 'h>(&'r self, haystack: &'h [u8]) -> Matches<'r, 'h> {
        Matches {
            haystack,
            it: self.pikevm.find_iter(self.pool.get(), haystack),
        }
    }
    ...
    ///     movies.push((title, std::str::from_utf8(year)?.parse::<i64>()?));
    /// }
    ...
    /// assert_eq!(got, vec![&b"Mary"[..], &b"had"[..], &b"a"[..], &b"little"[..], &b"lamb"[..]]);
    ///
    ...
    /// let got: Vec<&[u8]> = re.split(hay).collect();
    /// assert_eq!(got, vec![&b"lion"[..], &b""[..], &b"tiger"[..], &b"leopard"[..]]);
    ///
    /// let re = Regex::new(r"::").unwrap();
    ...
    /// let got: Vec<&[u8]> = re.split(hay).collect();
    /// assert_eq!(got, vec![&b""[..], &b""[..], &b""[..], &b""[..], &b"a"[..], &b""[..], &b"b"[..], &b"c"[..]]);
    ///
    /// let re = Regex::new(r"/").unwrap();
    /// let hay = b"(///)";
    /// let got: Vec<&[u8]> = re.split(hay).collect();
    ...
    /// let hay = b"rust";
    /// let got: Vec<&[u8]> = re.split(hay).collect();
    /// assert_eq!(got, vec![&b""[..], &b"r"[..], &b"u"[..], &b"s"[..], &b"t"[..], &b""[..]]);
    ///
    /// // Splitting by an empty string is UTF-8 aware by default!
    /// let re = Regex::new(r"").unwrap();
    /// let hay = "☃".as_bytes();
    /// let got: Vec<&[u8]> = re.split(hay).collect();
    /// assert_eq!(got, vec![b"", "☃".as_bytes(), b""]);
    /// ```
    ///
    ...
    /// a closure of type `|&Captures| -> Vec<u8>` provides direct access to the
    /// captures corresponding to a match. This allows one to access capturing
    /// group matches easily:
    ///
    /// ```
    /// use regex_lite::bytes::{Captures, Regex};
    ///
    /// let re = Regex::new(r"([^,\s]+),\s+(\S+)").unwrap();
    /// let result = re.replace(b"Springsteen, Bruce", |caps: &Captures| {
    ///     [&caps[2], b" ", &caps[1]].concat()
    /// });
    ...
    ///
    /// ```
    /// use regex_lite::bytes::{NoExpand, Regex};
    ///
    ...
    ///         new.extend(&haystack[last_match..m.start()]);
    ///         new.extend(&replacement(&caps)?);
    ///         last_match = m.end();
    ///     }
    ///     new.extend(&haystack[last_match..]);
    ///     Ok(new)
    /// }
    ///
...
impl<'h> Captures<'h> {
    ...
    pub fn extract<const N: usize>(&self) -> (&'h [u8], [&'h [u8]; N]) {
        ...
        let whole_match = matched.next().expect("a match").as_bytes();
        let group_matches = [0; N].map(|_| {
            matched.next().expect("too few matching groups").as_bytes()
        });
    ...
    pub fn expand(&self, replacement: &[u8], dst: &mut Vec<u8>) {
        interpolate::bytes(
            ...
            |index, dst| {
                ...
                dst.extend(&self.haystack[m.range()]);
            },
...
/// variants of string types, as well as `FnMut(&Captures) -> Vec<u8>` (or any
/// `FnMut(&Captures) -> T` where `T: AsRef<[u8]>`). Those cover most use cases,
...
///
/// ```
/// use regex_lite::bytes::{Captures, Regex, Replacer};
///
/// struct NameSwapper;
///
/// impl Replacer for NameSwapper {
///     fn replace_append(&mut self, caps: &Captures<'_>, dst: &mut Vec<u8>) {
///         dst.extend(&caps["first"]);
///         dst.extend(b" ");
///         dst.extend(&caps["last"]);
///     }
/// }
///
/// let re = Regex::new(r"(?<last>[^,\s]+),\s+(?<first>\S+)").unwrap();
...
pub trait Replacer {
    ...
    ///
    /// ```
    /// use regex_lite::bytes::{Regex, Replacer};
    ///
...
impl<'a> Replacer for &'a Vec<u8> {
    fn replace_append(&mut self, caps: &Captures<'_>, dst: &mut Vec<u8>) {
        self.as_slice().replace_append(caps, dst)
    }
...
impl RegexBuilder {
    ...
    ///
    /// ```
    /// use regex_lite::bytes::RegexBuilder;
    ///
    ...
    /// assert_eq!(Some(&b"J"[..]), caps.name("initial").map(|m| m.as_bytes()));
    ...
    /// assert_eq!(Some(&b"James"[..]), caps.name("middle").map(|m| m.as_bytes()));
    /// assert_eq!(b"Potter", &caps["last"]);
    /// ```
